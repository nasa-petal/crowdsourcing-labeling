...
labeling_job_name = 'ground-truth-petal-quadrant-test-9'
task_title = 'Multi-label text classification task of journal papers related to biomimicry: herb only test: test 9'
comment = 'test of script herb only test-test 9'
...
def get_list_of_abstracts_from_papers_using_kmeans(df_papers, num_clusters, num_papers_desired,
    ...
    model_abstracts = KMeans(n_clusters=num_clusters, init='k-means++', max_iter=100,
                                     n_init=1, random_state=0)  # need random_state to make
                                                                # deterministic
...
num_papers_to_be_labeled = len(papers_to_be_labeled)
...
labeling_job_arguments = {
    ...
    'HumanTaskConfig': {
        ...
        'UiConfig': {
            'UiTemplateS3Uri': f's3://{s3_bucket_name}/{custom_worker_task_template}'
        },
        ...
        'MaxConcurrentTaskCount': num_papers_to_be_labeled * num_workers_per_paper,  # text sent at a time to the workteam. Defines the maximum
                                       # number of data objects that can be labeled by human workers at the same time
                                       # see for great info https://docs.aws.amazon.com/sagemaker/latest/dg/sms-batching.html
...
# AWESOME document about doing this work !
#   https://sagemaker-examples.readthedocs.io/en/latest/ground_truth_labeling_jobs/ground_truth_object_detection_tutorial/object_detection_tutorial.html
